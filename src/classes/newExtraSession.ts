...
class Level {
    ...
    constructor(identifiant: number, newTabTdp: Tdp[]) {
        this.number = identifiant
        this.tdps = (newTabTdp.filter(tdp => tdp.level === identifiant))[0]//?
    }
}
...
function sorterData(tab: Tdp[]) {
    ...
    tab.forEach((e: Tdp) => {
        ...
        newTab.push(new Tdp({...e, position: parseInt(""+e.salle+e.rco+rectifyFerme.toString()+e.level)})) 
    })
...
export default class ExtraSession{
    ...
    constructor(sessionData: any[], repName: string){
        ...
        this.brutdata = sorterData(sessionData.map(elem=>{
            if (!elem.status) return new Tdp({...elem, status: "original"})
        ...
        const newTab : Tdp[] = []
        this.rep.forEach((elem: { salle: Salle[] })=>{
    ...
    modifRegType(itemId: string[], value: string){
        ...
        itemId.forEach((element: string) => {
            ...
            const monElem = {...this.brutdata[elem], regletteType:value}
    ...
    modifRegNbr(itemId: string[], value: string){
        ...
        itemId.forEach((element: string) => {
            ...
            const monElem = {...this.brutdata[i], regletteNbr:value }
