    ...
    render(){
        return (
            <Tab.Container  key={'Tab.Container'+this.props.data.number} id="left-tabs-example" defaultActiveKey="rco1" style={{backgroundColor:'white'}}>
                <Nav fill variant="pills" className="flex">{this.props.data.map(elem=><Nav.Item key={`Nav.Item rco${elem.number}`}><Nav.Link style={{padding: ".30rem"}} key={`rco${elem.number}`} eventKey={`rco${elem.number}`}>{`rco${elem.number}`}</Nav.Link></Nav.Item>)}</Nav>
